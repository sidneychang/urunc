...
func CreateSnapshotView(ctx context.Context, bundle, containerID string) (*SnapshotViewInfo, error) {
	...
	if err != nil {
		...
		// parent snapshot (when available). If that fails, treat it as a
		// hard error instead of falling back to the active snapshot key, to
		...
		if lerr != nil {
			...
			if rerr := ls.AddResource(ctx, lease, leases.Resource{
				...
				log.WithFields(logrus.Fields{
					...
					"view_key":  viewKey,
					"lease_id":  leaseID,
				}).Debug("created lease and attached snapshot view")
			}
		}
	}
