...
// non-loopback link that has an IP address and a default route (Dst == nil).
...
func deleteAllQDiscs(device netlink.Link) error {
	...
	// Try to discover the container interface instead of relying on DefaultInterface.
...
func deleteAllTCFilters(device netlink.Link) error {
	...
	// Try to discover the container interface to fetch its filters.
	device, err = discoverContainerIface()
	if err != nil {
		return err
	}
